...
class EnhancedMCPClient:
    ...
    async def _call_mcp_function(self, function_name: str, params: Dict) -> Dict:
        ...
        try:
            response = await self.session.post(
                f"{self.mcp_url}/tools/{function_name}",
                json=params
            )
    ...
    async def query_with_pagination(
        ...
        while True:
            ...
            logger.info(f"Querying MCP with cursor: {cursor[:20] if cursor else 'None'}...")
            
            try:
                response = await self._call_mcp_function(
                    "dshield-mcp:query_dshield_events",
                    params
                )
    ...
    async def stream_with_sessions(
        ...
        try:
            ...
            response = await self._call_mcp_function(
                "dshield-mcp:stream_dshield_events_with_session_context",
                params
            )
    ...
    async def get_entity_details(
        ...
        if fields is None:
            fields = [
                ...
                "event.action", "event.category", "@timestamp",
    ...
    async def get_data_dictionary(self) -> Dict:
        ...
        try:
            response = await self._call_mcp_function(
                "dshield-mcp:get_data_dictionary",
                {"format": "json"}
            )
            return response
        except Exception as e:
...
class MCPQueryOptimizer:
    ...
    async def identify_anomalies(
        ...
        logger.info(f"Identified {len(anomalies)} anomalies from {summary.total_events} events")
