...
class ConfigManager:
    ...
    def _load_env_config(self) -> dict[str, Any]:
        ...
        if os.getenv("VECTOR_DB_PATH"):
            env_config.setdefault("database", {})["vector_db_path"] = os.getenv(
                "VECTOR_DB_PATH"
            )
        ...
        if max_entities:
            env_config.setdefault("analysis", {})["max_entities_per_window"] = int(
                max_entities
            )
        anomaly_threshold = os.getenv("ANOMALY_THRESHOLD")
        if anomaly_threshold:
            env_config.setdefault("analysis", {})["anomaly_threshold"] = float(
                anomaly_threshold
            )
        ...
        if os.getenv("OLLAMA_BASE_URL"):
            env_config.setdefault("local_llm", {})["base_url"] = os.getenv(
                "OLLAMA_BASE_URL"
            )

        # API LLM configuration
        if os.getenv("CLAUDE_API_KEY"):
            env_config.setdefault("api_llm", {})["claude_api_key"] = os.getenv(
                "CLAUDE_API_KEY"
            )
            env_config.setdefault("api_llm", {})["use_api"] = True
        if os.getenv("OPENAI_API_KEY"):
            env_config.setdefault("api_llm", {})["openai_api_key"] = os.getenv(
                "OPENAI_API_KEY"
            )
            env_config.setdefault("api_llm", {})["use_api"] = True
        ...
        if enable_web_research:
            env_config.setdefault("enrichment", {})["enable_web_research"] = (
                enable_web_research.lower() == "true"
            )
        enable_threat_intel = os.getenv("ENABLE_THREAT_INTELLIGENCE")
        if enable_threat_intel:
            env_config.setdefault("enrichment", {})["enable_threat_intelligence"] = (
                enable_threat_intel.lower() == "true"
            )
        ...
        if max_concurrent:
            env_config.setdefault("performance", {})["max_concurrent_requests"] = int(
                max_concurrent
            )
        request_timeout = os.getenv("REQUEST_TIMEOUT_SECONDS")
        if request_timeout:
            env_config.setdefault("performance", {})["request_timeout_seconds"] = int(
                request_timeout
            )
    ...
    def validate_config(self, config: Config) -> bool:
        ...
        try:
            ...
            if config.api_llm.use_api:
                if (
                    not config.api_llm.claude_api_key
                    and not config.api_llm.openai_api_key
                ):
    ...
    def _config_to_dict(self, config: Config) -> dict[str, Any]:
        ...
        """
        return {
            "mcp": {"url": config.mcp.url, "timeout": config.mcp.timeout},
            "database": {
            ...
            "logging": {"level": config.logging.level, "file": config.logging.file},
            "metrics": {
