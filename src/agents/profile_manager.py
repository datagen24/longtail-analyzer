...
class AttackerProfile:
    ...
    def calculate_threat_score(self) -> float:
        ...
        avg_confidence = float(np.mean(list(self.confidence_scores.values())) if self.confidence_scores else 0)
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "persistence_indicators": [p.to_dict() for p in self.persistence_indicators],
...
class ProfileManager:
    ...
    def _init_db(self):
        ...
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_threat_level ON profiles(threat_level)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_entity_type ON profiles(entity_type)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_last_updated ON profiles(last_updated)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_relationships ON entity_relationships(entity1_id)")
    ...
    def save_profile(self, profile: AttackerProfile) -> bool:
        ...
        try:
            ...
            self.conn.commit()
            logger.info(f"Saved profile for {profile.entity_id} (threat level: {profile.threat_level.name})")
            return True
    ...
    def get_profile(self, entity_id: str) -> Optional[AttackerProfile]:
        ...
        try:
            ...
            for key, value in profile_data.items():
                if key in ["entity_id", "entity_type", "entity_value", "first_seen", "last_seen"]:
                    continue
                    
                if key == "threat_level":
                    profile.threat_level = ThreatLevel(value)
                elif key in ["scanning_patterns", "exploitation_attempts", "persistence_indicators"]:
    ...
    def _detect_attack_patterns(self, profile: AttackerProfile, events: List[Dict]):
        ...
        if scanning_events:
            self._update_pattern_list(profile.scanning_patterns, "scanning", scanning_events)
        if exploitation_events:
            self._update_pattern_list(profile.exploitation_attempts, "exploitation", exploitation_events)
        if persistence_events:
            self._update_pattern_list(profile.persistence_indicators, "persistence", persistence_events)
    
    def _update_pattern_list(
        self,
        pattern_list: List[AttackPattern],
        pattern_type: str,
        events: List[Dict]
    ):
        ...
        if existing_pattern:
            ...
            for event in events:
                if "destination.ip" in event and event["destination.ip"] not in existing_pattern.targets:
                    existing_pattern.targets.append(event["destination.ip"])
                if "destination.port" in event and event["destination.port"] not in existing_pattern.ports:
                    existing_pattern.ports.append(event["destination.port"])
        else:
            # Create new pattern
            targets = list(set(e.get("destination.ip", "") for e in events if e.get("destination.ip")))
            ports = list(set(e.get("destination.port", 0) for e in events if e.get("destination.port")))
    ...
    def _calculate_data_quality(self, profile: AttackerProfile) -> float:
        ...
        depth_scores = {"shallow": 0.2, "medium": 0.4, "deep": 0.6, "comprehensive": 0.8}
    ...
    def _calculate_similarity(self, profile1: Dict, profile2: Dict) -> float:
        ...
        if ports1 and ports2:
            port_similarity = len(ports1.intersection(ports2)) / len(ports1.union(ports2))
