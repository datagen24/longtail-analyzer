...
from src.agents.profile_manager import EntityType, ProfileManager
...
class TimeWindowProcessor:
    ...
    def generate_time_windows(
        ...
        while current < end_date:
            window_end = min(current + timedelta(hours=self.window_hours), end_date)
        ...
        logger.info(
            f"Generated {len(windows)} time windows from {start_date} to {end_date}"
        )
        return windows
    ...
    async def process_window(
        self, window: ProcessingWindow, pattern_analyzer=None
    ) -> dict:
        """
        ...
        try:
            ...
            logger.info(
                f"Window summary: {summary.total_events} events, "
            ...
            anomalies = await self.query_optimizer.identify_anomalies(
                summary, threshold_multiplier=2.0
            )
            ...
            logger.info(
                f"Identified {len(anomalies)} anomalous entities for detailed analysis"
            )
            ...
            if overlap_entities:
                await self._merge_cross_boundary_patterns(
                    overlap_entities, entity_events
                )
            ...
            return {"status": "error", "window_id": window.window_id, "error": str(e)}
    ...
    async def _merge_cross_boundary_patterns(
        self, overlap_entities: list[str], entity_events: dict[str, list[dict]]
    ):
        """
        ...
        for entity_id in overlap_entities:
            ...
            for event in current_events:
                ...
                for pattern in (
                    profile.scanning_patterns + profile.exploitation_attempts
                ):
    ...
    def load_checkpoint(self, analysis_id: str) -> dict | None:
    ...
    def _load_processed_windows(self):
        ...
        logger.info(
            f"Loaded {len(self.processed_windows)} previously processed windows"
        )
