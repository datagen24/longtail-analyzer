...
from src.models.pattern import Pattern, PatternSeverity, PatternType
...
class PatternAnalyzer:
    ...
    async def analyze(self, events: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        logger.debug(
            f"Pattern analysis complete: {len(analysis_results)} entities analyzed"
        )
    ...
    def _detect_scanning_patterns(self, events: list[dict[str, Any]]) -> list[Pattern]:
        ...
        for window_start, window_events in time_windows.items():
            ...
            ports = [
                e.get("destination.port")
                for e in window_events
                if "destination.port" in e
            ]
            if not ports:
                continue
            ...
            hosts = [
                e.get("destination.ip") for e in window_events if "destination.ip" in e
            ]
    ...
    def _detect_exploitation_patterns(
    ...
    def _detect_persistence_patterns(
        ...
        access_events = [
            e for e in events if e.get("event.action") in ["access", "login", "connect"]
        ]
    ...
    def _detect_anomalies(self, events: list[dict[str, Any]]) -> list[Pattern]:
        ...
        if intervals:
            ...
            burst_events = [
                i for i, interval in enumerate(intervals) if interval < burst_threshold
            ]
    ...
    def _calculate_confidence(
        self, events: list[dict[str, Any]], patterns: list[Pattern]
    ) -> float:
        """
    ...
    def _create_time_windows(
        self, events: list[dict[str, Any]], window_minutes: int = 5
    ...
    def _load_pattern_templates(self) -> dict[str, Any]:
        ...
        return {
            "scanning": {"min_events": 10, "min_ports": 20, "min_hosts": 20},
            "exploitation": {
    ...
    def _load_ttp_mappings(self) -> dict[str, str]:
        ...
        return {"scanning": "T1046", "exploitation": "T1059", "persistence": "T1078"}
