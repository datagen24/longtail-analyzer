...
class VectorStore:
    ...
    def add_pattern_embedding(
        ...
        try:
            ...
            # Add to collection
            self.collection.add(
                ids=[pattern_id], embeddings=[embedding_list], metadatas=[metadata]
            )
    ...
    def add_profile_embedding(
        self, profile_id: str, embedding: np.ndarray, metadata: dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            # Add to collection
            self.collection.add(
                ids=[profile_id], embeddings=[embedding_list], metadatas=[metadata]
            )
    ...
    def search_similar_patterns(
        ...
        try:
            ...
            results = self.collection.query(
                query_embeddings=[query_list], n_results=top_k, where=filter_metadata
            )
    ...
    def get_embedding(self, item_id: str) -> np.ndarray | None:
        ...
        """
        try:
            results = self.collection.get(ids=[item_id], include=["embeddings"])
