...
class StateStore:
    ...
    def _init_db(self) -> None:
        ...
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_sessions_type ON analysis_sessions (session_type)"
        )
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_checkpoints_session ON analysis_checkpoints (session_id)"
        )
        self.conn.execute(
    ...
    def get_latest_checkpoint(self, session_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            checkpoint_data["checkpoint_data"] = json.loads(
            ...
            logger.error(
                f"Error getting latest checkpoint for session {session_id}: {e}"
            )
            return None

    def save_analysis_result(
        self, session_id: str, result_type: str, result_data: dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            result_id = (
                f"{session_id}_{result_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            self.conn.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                (result_id, session_id, result_type, json.dumps(result_data)),
            )
    ...
    def get_analysis_results(
        self, session_id: str, result_type: str | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.error(
                f"Error getting analysis results for session {session_id}: {e}"
            )
            return []
    ...
    def set_system_state(
        ...
        try:
            self.conn.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                (key, json.dumps(value), datetime.now(), description),
            )
    ...
    def get_active_sessions(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for row in rows:
                ...
                # Parse JSON fields
                if session_data["config_data"]:
                    session_data["config_data"] = json.loads(
                        session_data["config_data"]
                    )
                if session_data["metadata"]:
                    session_data["metadata"] = json.loads(session_data["metadata"])
    ...
    def get_session_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            cursor = self.conn.execute(
                "SELECT COUNT(*) as count FROM analysis_checkpoints"
            )
