...
from .llm.api_llm import ClaudeAPI, OpenAIAPI
...
class LongTailAnalyzer:
                    ...
                    if threat_score > 0.7:
                        ...
                        if entity_id:
                            ...
                            if self.api_llm and quick_analysis.get("needs_deep_analysis"):
                                try:
                                    deep_analysis = await self.api_llm.analyze_events(events)
                                    quick_analysis.update(deep_analysis)
                                except Exception as e:
                                    logger.error(f"Error in API LLM analysis: {e}")
                            
                            # Update profile
                            updated_profile = self.profile_manager.update_profile_incrementally(
                                entity_id,
                                events,
                                quick_analysis
                            )
    ...
    async def enrich_profiles(self, entity_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        """
    ...
    def _extract_entity_id(self, events: List[Dict[str, Any]]) -> Optional[str]:
